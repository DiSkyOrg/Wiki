...
ParseSK is a simple but powerful Skript addon that enables you to programmatically parse and validate any Skript code from strings. You can check code for syntax errors, handle parsing errors gracefully, and optionally execute the parsed code—all at runtime.
...
discord command validate <text>:
    ...
    trigger:
        ...
        if {_errors::*} is not set:
            make embed:
                set title of embed to "✅ Valid Code"
            ...
            make embed:
                set title of embed to "❌ Syntax Errors"
...
command /runtest <text>:
    ...
    trigger:
        ...
        if {_errors::*} is not set:
            ...
            send "&c✗ Test failed with errors:"
            loop {_errors::*}:
                send "&c  - %loop-value%"
...
discord command learn <text>:
    ...
    trigger:
        ...
        make embed:
            if {_errors::*} is not set:
                set title of embed to "📚 Code Analysis"
                set description of embed to "✅ Your code is syntactically correct!"
                ...
                set title of embed to "📚 Code Analysis"
                set description of embed to "❌ Your code has errors. Let's fix them!"
                add field named "Code" with value "```applescript%nl%%{_code}%nl%```" to embed
...
discord command safeeval <text>:
    ...
    trigger:
        ...
        if {_code} contains "delete" or "op" or "execute console command":
            reply with "❌ Code contains prohibited operations!"
            stop
        ...
        if {_errors::*} is set:
            reply with "❌ Syntax errors detected. Code not executed."
            stop
        ...
        if {_errors::*} is set:
            reply with "❌ Execution failed: %join {_errors::*} with nl%"
        else:
            reply with "✅ Code executed successfully!"
```
